$(document).ready(function() {
    ...
    $('.saveSettings').click(function(){
        ...
        if(reg.test(str) == false && $('#formName').val() != 'generateforms') {
            $('#formName').parent().after('<p class="error">Form name can\'t be empty, can\'t called "generateforms" & can\'t contain spaces or special character (min : 3 char, max 40 chars)</p>');
            settingsError = 1;
        } 
        // Check css file name
    ...
    function build() {
        ...
        $.ajax({  
            url: $('#base_url').val()+"generateforms/builder",  
            dataType: 'json',  
            data: {
                'formName' : formName,
                'cssName' : cssName,
    ...
    function create_files() {
        ...
        // Call the ajax builder
        $.ajax({  
            url: $('#base_url').val()+"generateforms/creator",  
            dataType: 'json',  
            data: {
                'formName' : formName,
                'cssName' : cssName,
    ...
    function create_db() {
        ...
        // Call the ajax builder
        $.ajax({  
            url: $('#base_url').val()+"generateforms/dbCreator",  
            dataType: 'json',  
            data: {
                'formName' : formName, 
                'cssName' : cssName,
